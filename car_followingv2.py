...
from math import fabs, tanh, cos, sin, pi
...
class TrackTarget:
    ...
    def create_by_list(id, dx, dy, vx, vy):
        ...
        t  = TrackTarget(id, nx, ny, vx, vy, True, False)
        return t

    def __init__(self, id, dx, dy, vx, vy, updated, start):
        # 雷达数据
        self.id = id
        self.dx = dx
        self.dy = dy
    ...
    def __str__(self) -> str:
        return f'Target<id={self.id}, dy={int(self.dy)}, vy={int(self.vy)}, mdy={int(self.m_dy)} mvy={int(self.m_vy)} u={self.updated}, s={self.is_stop()} start={self.start}>'
...
class TraceModel:
    ...
    def on_new_frame(self, frame):
        ...
        for new_target in new_targets:
            ...
            # 用旧模型数据替换当前模型数据
            new_target.copy_model_from(matched_target)
            new_target.start = True
        ...
        for group in group_by_lane.values():
            ...
            if not self.should_start_parking_process(sorted_group[0]):
                sorted_group[0].update_model_from_radar()
                prev = sorted_group[0]
                for t in sorted_group[1:]:
                    if t.start:
                        if abs(t.dy - prev.m_dy) > self.car_length + 3:
                            if t.dy - t.m_dy < 0.5:
                                t.update_model_from_radar()
                            else:
                                t.m_dy += 1
                            # t.m_dy = (t.m_dy + prev.dy) / 2
                            t.start = True
                        else:
                            t.m_dy = t.m_dy + ((abs(t.dy - prev.m_dy) - self.car_length) / 2 ) 
                            t.start = True
                    else:
                        break
                    prev = t
                ...
                #     if t.vy < self.update_speed and not t.start:
                #         t.update_model_from_radar()
                continue
            # 在停车线附近
    ...
    def should_start_parking_process(self, t):
        ...
        return abs(t.dy - self.stop_lane) < self.car_length and abs(t.vy) < self.stop_speed
    ...
    def find_matched_stop_target(self, new_target):
        ...
        for tracked in self.tracked.values():
            ...
            #     continue
            if not tracked.is_stop():
                continue
            # 与 stop 目标匹配
            # if self.is_in_same_lane(new_target, tracked) and abs(new_target.dy - tracked.dy) < 1:
            if new_target.id == tracked.id or self.is_in_same_lane(new_target, tracked) and abs(new_target.dy - tracked.dy) < 1:
                return tracked
        return None
